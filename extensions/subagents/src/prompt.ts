...
export const SUBAGENT_SPAWN_PARAMETER_DESCRIPTIONS = {
  ...
  harness:
    'Harness to run the subagent on: "pi" (in-process pi session; inherits this environment), "claude" (Claude Code), "codex" (Codex CLI), or "gemini" (Antigravity CLI `agy`). Choose deliberately per task.',
  workingDir:
    "Trusted working directory for the autonomous child (default: current working directory)",
  model:
    'Model hint, interpreted by the chosen harness (pi: "provider/model-id" or model id; claude: model alias like "sonnet"/"opus"/"fable"; codex: model slug; gemini: `agy --model` value e.g. "gemini-3.1-pro-high"). Omit for the harness default (pi inherits the current model; gemini defaults to gemini-3.1-pro-high).',
  reasoningEffort:
    "Reasoning effort on a shared scale; the harness maps it to its nearest native equivalent (pi thinking level, codex reasoning effort, claude thinking budget; gemini/agy ignores this). Omit for the harness default (pi inherits the current level).",
};
